...
class QuadOptimizer:
    ...
    def robot_state_callback(self, data):
        ...
        ###TODO: pull pendulum data (vermutlich in anderem callback)
        s, r = None, None
        ds, dr = None, None
    ...
    def quadrotor_optimizer_setup(self, ):
        ...
        Q_m_ = np.diag([80.0, 80.0, 120.0,
                20.0, 20.0,
            	10.0, 10.0, 0.0,
                30.0, 20.0, 20.0,
                10.0, 10.0])  # position, velocity, roll, pitch, (not yaw)
        ...
        P_m_[2, 10] = 10.95
        P_m_[10, 2] = 10.95
        R_m_ = np.diag([50.0, 60.0, 1.0, 0.0])
        ...
        ocp.cost.Vx_e = np.zeros((nx-2, nx)) # only consider p and v
        ocp.cost.Vx_e[:nx-2, :nx-2] = np.eye(nx-2)

        # initial reference trajectory_ref
        x_ref = np.zeros(nx)
        x_ref_e = np.zeros(nx-2)
        u_ref = np.zeros(nu)
        u_ref[-1] = self.g
        ...
        # Set constraints
       ocp.constraints.lbu = np.array([constraints.roll_min, constraints.pitch_min, constraints.thrust_min, constraints.r_min, constraints.s_min])
        ocp.constraints.ubu = np.array([constraints.roll_max, constraints.pitch_max, constraints.thrust_max, constraints.r_max, constraints.s_max])
